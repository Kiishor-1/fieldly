...
export default function Subscriptions() {
  ...
  const handleSubscribe = async (subscription) => {
    ...
    try {
      await handleSubscriptionPayment(token, amount, subscription._id, user, async () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {loading ? (
        <div className="w-full h-[calc(100vh-243px)] flex items-center justify-center">
        ...
        <>
          <section>
            ...
            {subscriptions.userSubscriptions.length > 0 ? (
              ...
              <p className="text-gray-600">No subscriptions yet. Start subscribing to services!</p>
            )}
          </section>

          <section className="mt-10">
            <h2 className="text-2xl font-semibold mb-4 text-gray-700">Available Subscriptions</h2>
            {subscriptions.availableSubscriptions.length > 0 ? (
              <Swiper
                spaceBetween={20}
                slidesPerView={1}
                loop={true}
              ...
              <p className="text-gray-600">No available subscriptions at the moment.</p>
            )}
          </section>
        </>
      )}
